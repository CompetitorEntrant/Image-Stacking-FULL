...
#Description: This script calculates the SED by integrating the total flux within 100 kpc radii.
...
hdulist = fits.open('2175REFSUBComb13-g.fit')
scidata = hdulist[0].data.astype(float)
nmgy_count = 0

for i in range(len(scidata)):
    for j in range(len(scidata[0])):
        if scidata[i][j] > -0.001 and distance(i, j, 50, 50) >= 8 and distance(i, j, 50, 50) <= 30:
...
hdulist = fits.open('2175REFSUBComb13-r.fit')
scidata = hdulist[0].data.astype(float)
nmgy_count = 0
...
hdulist = fits.open('2175REFSUBComb13-i.fit')
scidata = hdulist[0].data.astype(float)
nmgy_count = 0
...
hdulist = fits.open('2175REFSUBComb13-z.fit')
scidata = hdulist[0].data.astype(float)
nmgy_count = 0

for i in range(len(scidata)):
    for j in range(len(scidata[0])):
        if scidata[i][j] > -0.005 and distance(i, j, 50, 50) >= 8 and distance(i, j, 50, 50) <= 30:
            nmgy_count += scidata[i][j]
...
plt.legend(handles=[MG_patch, Zibetti_patch])
ax.set_title('SED of Mg II Absorbers, 0.37 <= Zabs < 0.55', y=1.05, fontsize=20)
