...
#Description: This script is used to identify and download the reference quasars for the 2175 A dust absorber host galaxies as prescribed in the method section.
...
for i in range(len(dirs)):
    ...
    for j in range(len(tableDR12)):
        if abs(tableDUST[0]['ZABS'][dust_index] - tableDR12['Z_VI'][j]) < 0.1 and abs(tableDR12['PSFMAG'][catalog_index][2] - tableDR12['PSFMAG'][j][2]) < 0.5 and (j not in used):
